...
import ProductModalCustomer from "./ProductModalCustomer";
...
const PaymentModal = ({ show, onHide, entry, onPaymentSaved }) => {
  ...
  const fetchLastVoucherAndBank = async () => {
    try {
      ...
      const lastBankName = last?.bankdetails?.[0]?.Bankname || "";
      const lastBankCode = last?.bankdetails?.[0]?.code || "";

      setVoucherno(lastVoucherNo + 1);
      setBankName(lastBankName);
      setBankCode(lastBankCode);
    } catch (err) {
        ...
        items: [
          {
            ...
            acode:supplier.Vcode || 0,
            pan: supplier.pan || "",
            Add1: supplier.Add1 || "",
            bsGroup: supplier.bsGroup || "",
            payment_debit: formattedAmount,
            receipt_credit: "0.00",
...
{showBankModal && (
  <ProductModalCustomer
    onSelect={(product) => {
      ...
      setBankName(product.ahead || "");
      setBankCode(product.acode || "");
