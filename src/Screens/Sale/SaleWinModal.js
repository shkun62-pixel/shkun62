...
// export default function SaleWinModal({
//   open,
//   onClose,
...
//       salef3: String(form.salef3 || "").trim(),
...
//       if (isEdit) {
//         res = await axios.put(
//           `https://www.shkunweb.com/shkunlive/${tenant}/tenant/sale-win-update/${initialData._id}`,
//           payload
//         );
//       } else {
//         res = await axios.post(`https://www.shkunweb.com/shkunlive/${tenant}/tenant/sale-win-post`, payload);
//       }
...
//         `https://www.shkunweb.com/shkunlive/${tenant}/tenant/sale-win-delete/${initialData._id}`
//       );
...
//                 {isEdit ? "Edit Sale Window" : "Create Sale Window"}
//               </Typography>
...
//                   fullWidth
//                   value={form.salef3}
//                   onChange={(e) => setField("salef3", e.target.value)}
//                 />
//               </Grid>
...
//                   helperText="Example: /salegst, /sale, /sale-win etc."
//                 />
//               </Grid>
//             </Grid>
...
export default function SaleWinModal({ open, onClose }) {
  const { company } = useContext(CompanyContext);
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(
        `https://www.shkunweb.com/shkunlive/${tenant}/tenant/api/sale-win`
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit) {
        res = await axios.put(
          `https://www.shkunweb.com/shkunlive/${tenant}/tenant/sale-win-update/${editingId}`,
          payload
        );
      } else {
        res = await axios.post(
          `https://www.shkunweb.com/shkunlive/${tenant}/tenant/sale-win-post`,
          payload
        );
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `https://www.shkunweb.com/shkunlive/${tenant}/tenant/sale-win-delete/${editingId}`
      );
