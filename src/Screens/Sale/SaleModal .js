...
//     boxShadow: `10px 10px 15px rgb(8, 8, 8),
//     -10px -10px 15px rgba(0, 0, 0, 0.6),
//     inset 5px 5px 10px rgba(0, 0, 0, 0.2),
//     inset -5px -5px 10px rgba(4, 4, 4, 0.2)`,
//     // boxShadow: 24,
...
// const LOCAL_STORAGE_KEY = 'rowsData';
// const defaultRows = [
//     { name: 'SALE GST', path: '/Sale' },
//     { name: 'SALE GST SERVICE', path: '/SaleService' },
// ];
...
const API_UPDATE = (id) => `https://www.shkunweb.com/shkunlive/${tenant}/tenant/sale-win-update/${id}`; // PUT
  ...
  const isPrimaryRow = (row) => {
    ...
    return n === "SALE GST" || n === "SALE GST SERVICE";
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        setRows([]);
        console.error("sale-win load error:", e);
      } finally {
  ...
  const handleNavigation = (row) => {
    ...
    navigate(row.path, { state: { saleWin: row } });
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      console.error("sale-win update error:", e);
      // optional: alert
