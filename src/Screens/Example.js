...
const Example = () => {
  ...
  // Filters Transactions
  const [optionValues, setOptionValues] = useState({
    T10: true, // ✅ group by BsGroup toggle
  });
  const [showOptions, setShowOptions] = useState(false);   // ✅ For Options modal
  const [filterType, setFilterType] = useState("All");     // ✅ Debit / Credit / All
  const [filteredTransactions, setFilteredTransactions] = useState([]); // ✅ For filtered txns
  ...
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");
  const [groupedView, setGroupedView] = useState(false); // true when T10 is true
const [selectedBsGroup, setSelectedBsGroup] = useState(""); // store clicked Bsgroup
...
useEffect(() => {
  ...
  if (optionValues.T10) {
    if (selectedBsGroup) {
      // show only ledgers in this group
      filtered = filtered.filter(
        (ledger) => ledger.formData.Bsgroup === selectedBsGroup
      );
    } else {
      // show only one row per Bsgroup
      const seenGroups = new Set();
      filtered = filtered.filter((ledger) => {
        if (seenGroups.has(ledger.formData.Bsgroup)) return false;
        seenGroups.add(ledger.formData.Bsgroup);
        return true;
      });
    }
  }
  ...
  setFilteredLedgers(filtered);
  setSelectedIndex(0); // reset highlight
}, [ledgers, searchTerm, optionValues.T10, selectedBsGroup]);
...
useEffect(() => {
  const handleKeyDown = (e) => {
    // 1️⃣ If modal is open
    if (showModal) {
      if (e.key === "Escape") {
        setShowModal(false);       // ✅ Close modal
        setActiveRowIndex(0);      // ✅ Reset modal highlight
      } else if (e.key === "ArrowDown") {
        setActiveRowIndex((prev) =>
      ...
      return; // ⚠️ stop further handling
    }

    // 2️⃣ If modal is closed, handle ledger/group navigation
    if (!filteredLedgers.length) return;
    ...
    if (e.key === "ArrowDown") {
      ...
      if (optionValues.T10) {
        if (!selectedBsGroup) {
          setSelectedBsGroup(ledger.formData.Bsgroup);
          setSelectedIndex(0);
        } else {
          openLedgerDetails(ledger);
        }
      ...
      // Only go back to groups if modal is closed
      if (optionValues.T10 && selectedBsGroup) {
        setSelectedBsGroup("");
        setSelectedIndex(0);
      }
    }
...
//   if (optionValues.T10) {
//     if (!selectedBsGroup) {
//       // Step 1: First Enter on a Bsgroup row → show all ledgers in this Bsgroup
//       setSelectedBsGroup(ledger.formData.Bsgroup);
//       setSelectedIndex(0); // reset highlight for new list
//     } else {
//       // Step 2: Second Enter on a ledger row inside Bsgroup → open account statement
//       openLedgerDetails(ledger);
//     }
//   } else {
//     // normal mode, open ledger modal
//     openLedgerDetails(ledger);
//   }
// }
...
//   }, [filteredLedgers, selectedIndex, showModal]);
  ...
  {filteredLedgers.map((ledger, index) => (
    <tr
      ...
      onClick={() => {
        if (groupedView) {
          setSelectedBsGroup(ledger.formData.Bsgroup); // show all ledgers of this group
        } else {
          openLedgerDetails(ledger); // normal ledger modal
        }
      }}
      onMouseEnter={() => setSelectedIndex(index)}
    >
