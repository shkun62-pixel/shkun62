...
const LOCAL_STORAGE_KEY = 'rowsDataTDS';
const defaultRowsTDS = [
    { name: 'VOUCHER.1', path: '/TdsVoucher'},
    { name: 'VOUCHER.2', path: '/' },
    { name: 'VOUCHER.3', path: '/' },
    { name: 'VOUCHER.4', path: '/' },
];
const TdsModal = ({ isOpen, onClose, onNavigate }) => {
 ...
 // Load rows from localStorage or use default rows
    useEffect(() => {
        ...
        if (savedRows) {
            ...
            localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(defaultRowsTDS));
            setRows(defaultRowsTDS);
        }
    }, []);
  ...
  const startEdit = () => {
    ...
    return (
        ...
        >
            <StyledModal>
                <h2 style={{ textAlign: 'center', marginBottom: 20, color: 'black' }}>VOUCHER SERIES</h2>
