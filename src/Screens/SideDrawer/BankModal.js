...
const LOCAL_STORAGE_KEY = 'rowsDataBankVoucher';
const defaultRowsBank = [
    { name: 'BANK1', path: '/BankVoucher' },
    { name: 'ADVANCE VOUCHER', path: '/' },
    { name: 'REFUND VOUCHER', path: '/' },
 ...
 // Load rows from localStorage or use default rows
    useEffect(() => {
        ...
        if (savedRows) {
            ...
            localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(defaultRowsBank));
            setRows(defaultRowsBank);
        }
    }, []);
  ...
  const startEdit = () => {
    ...
    return (
        ...
        >
            <StyledModal>
                <h2 style={{ textAlign: 'center', marginBottom: 20, color: 'black' }}>BANK TRANSACTION</h2>
