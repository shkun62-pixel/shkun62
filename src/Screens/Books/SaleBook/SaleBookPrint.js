...
const SaleBookPrint = React.forwardRef(({
    ...
    //     if (tableData.date) row["Date"] = formatDate(formData.date);
    //     if (tableData.billno) row["Bill No."] = formData.vbillno || "";
    //     if (tableData.weight) {
    //     row["Qty"] = Number(entry.items?.reduce((sum, i) => sum + parseFloat(i.weight || 0), 0).toFixed(3));
    //     }
    //     if (tableData.pcs) {
    //      row["Pcs"] = Number(entry.items?.reduce((sum, i) => sum + parseFloat(i.pkgs || 0), 0).toFixed(3));
    //     }
    //     if (tableData.accountname) row["A/C Name"] = customer.vacode?.trim() || "";
    //     if (tableData.city) row["City"] = customer.city || "";
    //     if (tableData.gstin) row["Gstin"] = customer.gstno || "";
    //     if (tableData.netvalue) row["Net Sale"] = parseFloat(formData.sub_total) || 0;
    //     if (tableData.cgst) row["C.Tax"] = parseFloat(formData.cgst) || 0;
    //     if (tableData.sgst) row["S.Tax"] = parseFloat(formData.sgst) || 0;
    //     if (tableData.igst) row["I.Tax"] = parseFloat(formData.igst) || 0;
    //     if (tableData.value) row["Bill Amount"] = parseFloat(formData.grandtotal) || 0;

    //     for (let i = 1; i <= 5; i++) {
    //       if (tableData[`exp${i}`]) {
    //         row[`Exp ${i}`] = parseFloat(item[`Exp${i}`]) || 0;
    //       }
    //     }
    //     for (let i = 6; i <= 10; i++) {
    //       if (tableData[`exp${i}`]) {
    //         row[`Exp ${i}`] = parseFloat(formData[`Exp${i}`]) || 0;
    //       }
    //     }

    //     if (tableData.description) row["Description"] = item.sdisc || "";
    //     if (tableData.vehicleno) row["VehicleNo"] = formData.trpt || "";
    //     if (tableData.remarks) row["Remarks"] = formData.rem2 || "";
    //     if (tableData.transport) row["Transport"] = formData.v_tpt || "";
    //     if (tableData.broker) row["Broker"] = formData.broker || "";
    //     if (tableData.taxtype) row["TaxType"] = formData.stype || "";
    ...
    //   // Align Bill No. left
    //   const billNoColIndex = header.indexOf("Bill No.");
    //   if (billNoColIndex !== -1) {
    //     const totalRows = exportData.length + 5;
    //     for (let rowIdx = 4; rowIdx < totalRows; rowIdx++) {
    //       const cellAddress = XLSX.utils.encode_cell({ r: rowIdx, c: billNoColIndex });
    ...
    //       for (let r = 5; r < exportData.length + 5; r++) {
    //         const cell = XLSX.utils.encode_cell({ r, c: colIdx });
    //         if (worksheet[cell]) worksheet[cell].z = "0.000"; // 3 decimal places
    //       }
    ...
    //       for (let r = 5; r < exportData.length + 5; r++) {
    //         const cell = XLSX.utils.encode_cell({ r, c: colIdx });
    //         if (worksheet[cell]) worksheet[cell].z = "0.00"; // 2 decimal places
    //       }
