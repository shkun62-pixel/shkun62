...
const SaleSumm = () => {
  ...
  useEffect(() => {
    const fetchAccountWiseData = async () => {
      try {
        ...
        faData.forEach((entry) => {
          entry.transactions
            .filter((txn) => txn.vtype === "S")
            .forEach((txn) => {
              const txnDate = new Date(txn.date);
              if (txnDate < fromDate || txnDate > toDate) return;
              ...
              if (txn.type === "debit") {
                accountTotals[acc].debit += txn.amount;
              } else if (txn.type === "credit") {
                accountTotals[acc].credit += txn.amount;
              }
            });
        });

        const tableRows = Object.entries(accountTotals).map(
          ([account, values]) => ({
            account,
            debit: values.debit,
        ...
        console.error("Account wise summary error:", err);
      }
    };

    fetchAccountWiseData();
  }, []);

  return (
    <div className="card p-3">
      <h5>Account Wise Sale Summary (FY 2025–26)</h5>
