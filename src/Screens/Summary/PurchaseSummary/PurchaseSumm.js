...
const PurchaseSumm = () => {
  ...
  const fetchPurSummary = async () => {
    try {
      ...
      data.forEach((voucher) => {
        ...
        // 🔹 Purchase ITEMS
        items.forEach((item) => {
          const key = `Pur_${item.Pcodes01}`;

          if (!saleMap[key]) {
            saleMap[key] = {
              account: item.Pcodes01, // 🔥 CODE
              accountName: item.Pcodess, // 🔥 NAME
              pcs: 0,
              qty: 0,
              value: 0,
              cgst: 0,
  ...
  return (
    <div className={styles.cardContainer}>
      <Card className={styles.cardSumm}>
        <h1 className={styles.headerSummary}>PURCHASE SUMMARY</h1>
        <div
          style={{
            display: "flex",
            flexDirection: "row",
        ...
        <Card.Footer className="fw-bold" style={{ fontSize: "20px",display:'flex', flexDirection:'row', justifyContent:'space-between' }}>
          Total Purchase Rs. {totalSale.toFixed(2)}
          <Button
