...
const PurWin = ({ isOpen, onClose }) => {
  const [formData, setFormData] = useState([]);

  // Load Data
  useEffect(() => {
    const stored = localStorage.getItem("PurWin");
  ...
  const handleSave = () => {
    ...
    localStorage.setItem("PurWin", JSON.stringify(filtered));
  ...
  return (
    <Modal open={isOpen} onClose={onClose} style={{zIndex: 100000}}>
      <StyledModal>
        ...
        {/* Header */}
        <Header>
          <Typography variant="h6">Purchase Series</Typography>
