...
export default function PurFAVoucherModal({
  open,
  onClose,
  tenant,
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setFa(null);
      try {
        const url = `https://www.shkunweb.com/shkunlive/shkun_05062025_05062026/tenant/purchasefaFile/byVoucher?vtype=${encodeURIComponent(
          vtype
  ...
  // print just preview
  const handlePrint = () => {
    const preview = document.getElementById("pur-fa-preview");
    if (!preview) return;
  ...
  // scoped CSS so on-screen looks same as print (and doesn’t leak)
    const getScopedCss = () => `
    #pur-fa-preview * { box-sizing:border-box; font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; }
    #pur-fa-preview .wrap { width:100%; background:#fff; }
    #pur-fa-preview .topbar {
    ...
    #pur-fa-preview .brand-block { text-align:left; }
    #pur-fa-preview .brand { font-size:20px; font-weight:800; letter-spacing:.3px; }
    #pur-fa-preview .sub { font-size:12px; color:#475569; }
    #pur-fa-preview .heads { text-align:right; font-size:12px; color:#374151; }
    #pur-fa-preview .badge {
    ...
    #pur-fa-preview .meta .box {
    flex:1; border:1px dashed #cbd5e1; border-radius:10px; padding:10px 12px; font-size:12px; color:#1f2937; background:#f8fafc;
    }
    #pur-fa-preview .meta .box strong { color:#111827; }

    #pur-fa-preview table { width:100%; border-collapse:collapse; }
    #pur-fa-preview thead th {
    position:sticky; top:0;
    background:#0ea5e9; color:#fff; font-weight:800; font-size:12px;
    padding:10px; border:1px solid #89cff0;
    }
    #pur-fa-preview tbody td {
    border:1px solid #e5e7eb; padding:8px 10px; font-size:12px; vertical-align:top;
    }
    #pur-fa-preview tbody tr:nth-child(odd) { background:#fafafa; }
    #pur-fa-preview .desc { width:60%; }
    #pur-fa-preview .num  { text-align:right; width:20%; white-space:nowrap; }

    #pur-fa-preview tfoot td {
    border:1px solid #cbd5e1; padding:10px; font-weight:800; font-size:12px; background:#f1f5f9;
    }
    #pur-fa-preview .totals-bar {
    display:flex; gap:12px; margin-top:10px;
    }
    #pur-fa-preview .totcard {
    flex:1; border:1px solid #e2e8f0; border-radius:12px; padding:10px 12px; background:#ffffff;
    display:flex; align-items:center; justify-content:space-between; font-size:13px;
    }
    #pur-fa-preview .status {
    font-size:12px; font-weight:800; padding:4px 10px; border-radius:999px; border:1px solid;
    }
    #pur-fa-preview .ok { background:#ecfdf5; color:#065f46; border-color:#a7f3d0; }
    #pur-fa-preview .bad { background:#fef2f2; color:#991b1b; border-color:#fecaca; }

    #pur-fa-preview .signs { display:flex; justify-content:space-between; margin-top:36px; font-size:12px; color:#6b7280; }
    #pur-fa-preview .line { min-width:28%; text-align:center; }
    `;
    ...
    <Modal open={open} onClose={onClose}>
      <Box sx={modalSx}>
        ...
        >
          ...
          {!loading && !error && fa && (
            <div id="pur-fa-preview" className="wrap">
              {/* Top ribbon */}
