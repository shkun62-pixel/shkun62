 ...
 */
// const API_LIST = "/tenant/api/purchase-win"; // GET
// const API_UPDATE = (id) => `/tenant/api/purchase-win/${id}`; // PUT

const PurModal = ({ isOpen, onClose, onNavigate }) => {
...
const API_LIST = `https://www.shkunweb.com/shkunlive/${tenant}/tenant/api/purchase-win`; // GET
const API_UPDATE = (id) => `https://www.shkunweb.com/shkunlive/${tenant}/tenant/purchase-win-update/${id}`; // PUT
  ...
  const isPrimaryRow = (row) => {
    ...
    return n === "purchase GST" || n === "purchase GST SERVICE";
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        setRows([]);
        console.error("purchase-win load error:", e);
      } finally {
  ...
  const handleNavigation = (row) => {
    ...
    navigate(row.path, { state: { purchaseWin: row } });
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      console.error("purchase-win update error:", e);
      // optional: alert
