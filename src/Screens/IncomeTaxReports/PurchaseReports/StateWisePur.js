...
export default function StateWisePur({ show, onClose }) {
  ...
  function summarizeByAccount(purchases) {
    ...
    purchases.forEach((p) => {
      p.items.forEach((item) => {
        ...
        const acc = p.supplierdetails?.[0]?.state || "UNKNOWN";
  ...
  function summarizeByMonth(purchases) {
    ...
    purchases.forEach(p => {
      ...
      p.items.forEach(item => {
        ...
        const account = p.supplierdetails?.[0]?.state || "UNKNOWN";
  ...
  const exportToExcel = () => {
    ...
    const title = `State Wise Purchase Summary - From: ${fromDate} To: ${toDate}`;
    ...
    XLSX.writeFile(wb, "StateWisePur.xlsx");
  };
      ...
      <Modal.Body style={{ maxHeight: "calc(100vh - 120px)", overflowY: "auto" }}>
        {fetching ? (
          ...
          <AccountWisePrint
            ...
            tittle = {"STATE WISE PURCHASE SUMMARY"}
          />
        )}
      </Modal.Body>
