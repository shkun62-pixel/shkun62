...
export default function BrokerWiseSale({ show, onClose }) {
  ...
  function summarizeByAccount(purchases) {
    ...
    purchases.forEach((p) => {
      p.items.forEach((item) => {
        const acc = p.formData?.broker || "UNKNOWN";
  ...
  function summarizeByMonth(purchases) {
    ...
    purchases.forEach(p => {
      ...
      p.items.forEach(item => {
        const account = p.formData?.broker || "UNKNOWN";
  ...
  const exportToExcel = () => {
    ...
    const title = `Broker Wise Sale Summary - From: ${fromDate} To: ${toDate}`;
    ...
    // Append Sheet & Save
    // ----------------------------
    XLSX.utils.book_append_sheet(wb, ws, "BrokerWise Summary");
    XLSX.writeFile(wb, "BrokerWiseSale.xlsx");
  };
